...
for phoneme in unique_phonemes:
    X_train_p = train_df[train_df['phoneme'] == phoneme][formant_features]
    y_train_p = train_df[train_df['phoneme'] == phoneme]['height_cm']
    X_test_p = test_df[test_df['phoneme'] == phoneme][formant_features]
    ...
    phoneme_results.append({
        'phoneme': phoneme,
        'rmse': rmse_p,
        **{f: imp for f, imp in zip(formant_features, importances)}
    })
...
plt.title("Formants Feature Importance per Phoneme for Height Prediction")
plt.xlabel("Formants Feature")
plt.ylabel("Phoneme")
plt.tight_layout()
plt.show()
...
mean_rmse_per_phoneme.to_csv("formants_rf_regression_results.csv", index=False)
